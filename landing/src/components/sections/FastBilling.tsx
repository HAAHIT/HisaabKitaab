...
export function FastBilling() {
  ...
  return (
    <section id="fast-billing" className="relative py-24 sm:py-32 overflow-hidden">
      <div className="absolute -top-40 left-0 w-[500px] h-[500px] bg-[radial-gradient(closest-side,rgba(40,224,168,0.10),transparent_70%)] blur-3xl" aria-hidden />
        ...
        <div className="order-2 lg:order-1">
          <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            ...
            <h2 className="font-display text-[36px] sm:text-[52px] font-medium leading-[1.02] tracking-tight mt-4">
              ...
              <span className="text-white/55">In the time it takes to pour chai.</span>
            </h2>
          ...
          <div className="mt-10 space-y-1">
            {steps.map((s, i) => (
              <motion.div
                ...
                className="group flex gap-5 py-5 border-t border-white/8"
              >
                <span className="font-mono text-[12px] text-[#28e0a8]/80 pt-1">{s.n}</span>
                <div className="flex-1">
                  <h3 className="text-[18px] font-medium text-white">{s.title}</h3>
                  <p className="mt-1 text-[14px] text-white/55 leading-relaxed">{s.body}</p>
                </div>
              </motion.div>
            ))}
          </div>
...
function BillingFlowVisual() {
  return (
    <div className="relative">
      <div className="absolute -inset-10 bg-[radial-gradient(closest-side,rgba(40,224,168,0.18),transparent_70%)] blur-3xl" aria-hidden />
      <div className="relative card shine p-5 sm:p-7">
        <div className="flex items-center justify-between text-[11px] text-white/50 mb-4">
          <span className="font-mono uppercase tracking-wider text-[#28e0a8]/80">Live: bill timer</span>
        ...
        <div className="mt-6 rounded-2xl bg-gradient-to-br from-[#28e0a8]/15 to-[#16b88a]/5 ring-1 ring-[#28e0a8]/20 p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[11px] text-white/55">WhatsApp message preview</p>
              <p className="mt-1 text-[13.5px] text-white">Namaste Ramesh ji, aapka bill ₹17,030 ka taiyaar hai.</p>
            </div>
            <span className="font-mono text-[10px] text-[#4fe9b9] bg-[#28e0a8]/10 rounded-full px-2 py-1 ring-1 ring-[#28e0a8]/20">UPI inside</span>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <motion.div
      ...
      className={`flex items-center gap-4 rounded-xl px-3.5 py-2.5 ring-1 ${
        success ? "bg-emerald-500/[0.07] ring-emerald-400/20" :
        highlight ? "bg-white/[0.04] ring-white/10" :
        "bg-white/[0.02] ring-white/[0.05]"
      }`}
    >
      <span className={`font-mono text-[11px] tabular-nums ${success ? "text-emerald-300" : "text-white/40"}`}>{time}</span>
      <span className={`text-[13px] ${success ? "text-emerald-100" : muted ? "text-white/45" : "text-white/85"}`}>{label}</span>
    </motion.div>
  );
}
