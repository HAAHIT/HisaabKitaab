...
export function NumbersSection() {
  return (
    <section className="relative py-24 sm:py-28 border-y border-white/5">
          ...
          {numbers.map((x, i) => (
            ...
            >
              <p className="font-display text-[44px] sm:text-[60px] leading-none font-medium tracking-tight">
                <span className="bg-gradient-to-br from-white to-[#28e0a8]/80 bg-clip-text text-transparent">{x.n}</span>
              </p>
              <p className="mt-3 text-[13px] text-white/75 leading-snug max-w-[200px]">{x.label}</p>
              <p className="mt-1 text-[11px] text-white/40 font-mono">{x.sub}</p>
            </motion.div>
          ))}
        </div>
      </div>
