...
## 2026-06-27 - Host Header Injection Vulnerability
**Vulnerability:** Host Header Injection via x-forwarded-host in public-url.ts
**Learning:** Absolute URLs generated dynamically from `request.headers` for password resets or redirects can be poisoned by malicious client headers.
**Prevention:** Prioritize statically configured and trusted environment variables like `process.env.NEXT_PUBLIC_SITE_URL` over incoming headers to establish the base URL securely.
