...
export function TallySection() {
  return (
    <section id="tally" className="relative py-24 sm:py-32 overflow-hidden border-t border-slate-200/60">
      <div className="absolute inset-0 bg-grid opacity-30" aria-hidden />
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#2563eb]/20 to-transparent" aria-hidden />
      <div className="relative mx-auto max-w-7xl px-5 sm:px-8">
        ...
        >
          ...
          <h2 className="font-display text-[36px] sm:text-[56px] font-medium leading-[1.02] tracking-tight mt-4 text-slate-900">
            One tap. <span className="text-slate-400">Your CA&apos;s inbox.</span>
          </h2>
          <p className="mt-5 text-[16.5px] text-slate-500 leading-relaxed">
            Every &quot;paisa mila&quot; silently writes a balanced double-entry voucher in the background. At quarter-end, hit <span className="text-slate-700">Export</span> and SoloBooks hands your Chartered Accountant a Tally XML that imports without errors — every time. Round-trip Tally Prime support means CA edits flow back in too.
          </p>
        </motion.div>
...
function Pillar({ icon, title, body }: { icon: React.ReactNode; title: string; body: string }) {
  return (
    ...
    >
      <div className="h-9 w-9 rounded-lg bg-[#2563eb]/[0.08] ring-1 ring-[#2563eb]/15 text-[#2563eb] grid place-items-center">
        {icon}
      </div>
      <h3 className="mt-4 text-[16px] font-medium text-slate-800">{title}</h3>
      <p className="mt-1.5 text-[13px] text-slate-500 leading-relaxed">{body}</p>
    </motion.div>
  );
}
