...
export default function EditTemplatePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    fetch(`/api/templates/${id}`)
      .then((res) => {
        if (!res.ok) throw new Error(t("templates.notFound" as TranslationKey));
        return res.json();
      })
      .then((data) => {
  ...
  function validateAll(): boolean {
    ...
    let valid = true;
    if (!name.trim()) { showToast(t("templates.nameRequired" as TranslationKey), "error"); return false; }
    const colNames = new Set<string>();
    columns.forEach((col, i) => {
      if (!col.name.trim()) { newErrors[i] = t("templates.columnRequired" as TranslationKey); valid = false; return; }
      if (colNames.has(col.name)) { newErrors[i] = t("templates.duplicateColumn" as TranslationKey); valid = false; return; }
      colNames.add(col.name);
      if (col.type === "formula") {
        if (!col.formula?.trim()) { newErrors[i] = t("templates.formulaRequired" as TranslationKey); valid = false; return; }
        const result = validateFormula(col.formula, col.name, columns);
        if (!result.valid) { newErrors[i] = result.error || t("templates.invalidFormula" as TranslationKey); valid = false; }
      }
    });
    setErrors(newErrors);
    return valid;
  ...
  async function handleSave() {
    ...
    try {
      ...
      showToast(t("templates.updatedSuccess" as TranslationKey), "success");
      setTimeout(() => router.push("/settings/templates"), 500);
    } catch (err) {
      showToast(err instanceof Error ? err.message : t("templates.updateFailed" as TranslationKey), "error");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <>
      ...
      <div style={{ fontFamily: SG }}>
        <PageHeader
          title={t("templates.editTitle" as TranslationKey)}
          subtitle={t("templates.editSubtitle" as TranslationKey)}
          isMobile={isMobile}
          action={
            <button
        ...
        <div>
          ...
          <HKCard style={{ marginBottom: 20 }}>
            <HKInput
              label={t("templates.templateName" as TranslationKey)}
          ...
          <HKCard style={{ marginBottom: 20 }}>
            <p style={{ fontSize: TYPE.h2, fontWeight: 700, color: "var(--sb-text)", fontFamily: SG, margin: "0 0 20px" }}>
              {t("templates.columns" as TranslationKey)}
            </p>
                ...
                >
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                    ...
                    <div style={{ flex: 1, display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 12 }}>
                      <HKInput
                        label={t("templates.columnName" as TranslationKey)}
                        placeholder={t("templates.columnNamePlaceholder" as TranslationKey)}
                        value={col.name}
                      ...
                      />
                      <HKSelect
                        label={t("templates.type" as TranslationKey)}
                        placeholder={t("templates.type" as TranslationKey)}
                        value={col.type}
                        onValueChange={(val) => { if (val) updateColumn(index, "type", val); }}
                        size="sm"
                      >
                        {COLUMN_TYPES.map((tItem) => (
                          <HKSelectItem key={tItem.key} value={tItem.key}>
                            {t(`templates.type.${tItem.key}` as TranslationKey)}
                          </HKSelectItem>
                        ))}
                      </HKSelect>
                    </div>
                  ...
                  {col.type === "formula" && (
                    <div style={{
                      ...
                      <p style={{ fontSize: TYPE.bodySmall, fontWeight: 700, color: AM, fontFamily: SG, margin: "0 0 10px", display: "flex", alignItems: "center", gap: 6 }}>
                        ...
                        {t("templates.buildFormula" as TranslationKey)}
                      </p>
                      <HKInput
                        aria-label="Formula expression"
                        placeholder={t("templates.formulaPlaceholder" as TranslationKey)}
                        value={col.formula || ""}
                        onValueChange={(v) => updateColumn(index, "formula", v)}
                        isInvalid={!!errors[index]}
                        errorMessage={errors[index] || t("templates.formulaHelp" as TranslationKey)}
                      />
                      <div style={{ marginTop: 12 }}>
                        <p style={{ fontSize: TYPE.caption, color: "var(--sb-sub)", fontFamily: SG, marginBottom: 8, fontWeight: 600 }}>
                          {t("templates.insertColumns" as TranslationKey)}
                        </p>
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                          {columns.slice(0, index).filter((c) => c.name.trim()).length > 0 ? (
                            ...
                            <span style={{ fontSize: TYPE.caption, color: "var(--sb-sub)", fontFamily: SG, fontStyle: "italic" }}>
                              {t("templates.noPreviousColumns" as TranslationKey)}
                            </span>
                          )}
                        </div>
                      </div>
                  ...
                  {col.type === "dropdown" && (
                    <div style={{
                      ...
                      <p style={{ fontSize: TYPE.bodySmall, fontWeight: 700, color: PU, fontFamily: SG, margin: "0 0 10px", display: "flex", alignItems: "center", gap: 6 }}>
                        ...
                        {t("templates.dropdownOptions" as TranslationKey)}
                      </p>
                      <HKInput
                        aria-label="Dropdown options"
                        placeholder={t("templates.dropdownPlaceholder" as TranslationKey)}
                        ...
                        description={t("templates.dropdownDescription" as TranslationKey)}
                      />
                    </div>
                  )}
            ...
            >
              ...
              {t("templates.addColumn" as TranslationKey)}
            </button>
          </HKCard>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 12 }}>
            <HKButton variant="secondary" onClick={() => router.push("/settings/templates")}>
              {t("common.cancel" as TranslationKey) || "Cancel"}
            </HKButton>
            <HKButton onClick={handleSave} isLoading={saving}>
              {t("templates.updateBtn" as TranslationKey)}
            </HKButton>
          </div>
        </div>
      </div>
