...
export async function GET(
  ...
  if (asset.storageProvider === "proxy") {
    try {
      ...
      // Node.js `URL.hostname` preserves surrounding brackets for IPv6 addresses
      // like `[::1]`. We explicitly strip them here so we compare the bare
      // textual host. We intentionally only block hosts that *parse* as a
      // private literal — DNS rebinding (a public hostname that resolves to a
      // private address) is mitigated separately by running the proxy in a
      // network namespace without metadata access. A safer alternative is
      // dns.lookup + re-check after the fetch handshake, but that requires a
      // custom http.Agent and is out of scope here.
